...
final class CliArgumentFailureSupport {
  ...
  private static final Map<String, List<String>> COMMAND_TEMPLATES_BY_FLAG =
      Map.ofEntries(
          ...
          Map.entry("--print-recipe", List.of("gridgrind --print-recipe --lookup WORKBOOK_HEALTH")),
          Map.entry("--print-recipe-catalog", List.of("gridgrind --print-recipe-catalog")),
          Map.entry(
              "--print-recipe-keyword-match",
              List.of(
                  "gridgrind --print-recipe-keyword-match --query \"monthly sales dashboard\"")),
          Map.entry("--print-protocol-catalog", List.of("gridgrind --print-protocol-catalog")),
          Map.entry(
              "--lookup",
              List.of(
          ...
          Map.entry(
              ...
              List.of(
                  "gridgrind --print-recipe-keyword-match --query \"monthly sales dashboard\"")),
          Map.entry("--search", List.of("gridgrind --print-protocol-catalog")),
  ...
  static CliDiagnostic reportFor(String[] args, CliArgumentsException exception) {
  ...
  static CliDiagnostic reportFor(String[] args, IllegalArgumentException exception) {
    Objects.requireNonNull(args, "args must not be null");
    Objects.requireNonNull(exception, "exception must not be null");
    String message =
        Objects.requireNonNullElse(exception.getMessage(), "Invalid command-line arguments");
  ...
  private static List<String> argumentSuggestions(String argument, String message) {
    ...
    switch (argument) {
      ...
      case "--lookup" -> {
        suggestions.add("gridgrind --print-recipe --lookup WORKBOOK_HEALTH");
        suggestions.add("gridgrind --print-recipe --lookup DASHBOARD");
        suggestions.add("gridgrind --print-recipe-catalog");
        suggestions.add("gridgrind --print-protocol-catalog");
      }
      case "--query" -> {
        suggestions.add(
            "gridgrind --print-recipe-keyword-match --query \"monthly sales dashboard\"");
