...
- Circular formulas are unevaluable: `REQUIRE_EVALUATION` fails with
  `CIRCULAR_FORMULA_REFERENCE`, while `EVALUATE_ALL` and `EVALUATE_TARGETS` remain partial and
  emit formula warnings. Ordinary evaluated Excel errors such as `#DIV/0!` remain evaluable
  results.
