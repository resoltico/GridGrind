...
| Scalar formula assignment, workbook-wide evaluation, targeted evaluation, cache clearing, and recalc-on-open | Documented formula and evaluation support | `COMPLETE` | `protocol/dto/CellInput.java`, `protocol/dto/FormulaCellTargetInput.java`, `protocol/operation/WorkbookOperation.java`, `engine/excel/ExcelWorkbook.java`, `engine/src/test/java/dev/erst/gridgrind/excel/ExcelFormulaEnvironmentTest.java`, `protocol/src/test/java/dev/erst/gridgrind/protocol/exec/FormulaEnvironmentRequestExecutorTest.java` | GridGrind supports whole-workbook evaluation, explicit-cell evaluation, explicit persisted-cache clearing, and recalc-on-open for the scalar formula surface it models publicly. Request-authored array-formula braces such as `{=SUM(A1:A10*B1:B10)}` are rejected as `INVALID_FORMULA`, and newer constructs such as `LAMBDA` or `LET` may also fail at parse time when Apache POI cannot parse them. Loaded formulas that POI parses but cannot evaluate surface as `UNSUPPORTED_FORMULA`. |
...
| Formula surface summaries | GridGrind-derived read over POI formula facts | `COMPLETE` | `protocol/read/WorkbookReadOperation.java`, `engine/excel/WorkbookAnalyzer.java`, `protocol/src/test/java/dev/erst/gridgrind/protocol/exec/DefaultGridGrindRequestExecutorTest.java`, `examples/introspection-analysis-request.json` | `GET_FORMULA_SURFACE` groups formula usage across one or more sheets without mutating the workbook. |
| Formula health analysis | GridGrind-derived analysis over POI formula facts | `COMPLETE` | `protocol/read/WorkbookReadOperation.java`, `engine/excel/WorkbookAnalyzer.java`, `engine/src/test/java/dev/erst/gridgrind/excel/WorkbookAnalyzerTest.java`, `examples/introspection-analysis-request.json` | `ANALYZE_FORMULA_HEALTH` reports volatile usage, formula-error results, and evaluation failures across the selected sheets. |
...
| Named-range surface summaries | GridGrind-derived read over POI named-range facts | `COMPLETE` | `protocol/read/WorkbookReadOperation.java`, `engine/excel/WorkbookAnalyzer.java`, `protocol/src/test/java/dev/erst/gridgrind/protocol/exec/DefaultGridGrindRequestExecutorTest.java`, `examples/introspection-analysis-request.json` | `GET_NAMED_RANGE_SURFACE` summarizes named-range scope and backing kind without mutating the workbook. |
| Named-range health analysis | GridGrind-derived analysis over POI named-range facts | `COMPLETE` | `protocol/read/WorkbookReadOperation.java`, `engine/excel/WorkbookAnalyzer.java`, `engine/src/test/java/dev/erst/gridgrind/excel/WorkbookAnalyzerTest.java`, `examples/introspection-analysis-request.json` | `ANALYZE_NAMED_RANGE_HEALTH` reports broken references, unresolved targets, and scope-shadowing findings. |
