...
- threaded comments
- array formulas as a dedicated public contract
- sparklines
...
|:-----------|:------------------|:-----------------|:-------------------|:------|
| `.xlsx` workbook create, open, save | Documented usermodel support | `COMPLETE` | `contract/dto/WorkbookPlan.java`, `engine/excel/ExcelWorkbook.java`, `engine/src/test/java/dev/erst/gridgrind/excel/ExcelWorkbookTest.java`, `executor/src/test/java/dev/erst/gridgrind/executor/DefaultGridGrindRequestExecutorTest.java` | GridGrind hard-restricts the product contract to `.xlsx`. |
| Sheet create, rename, delete, move | Documented guide/API support | `COMPLETE` | `contract/action/MutationAction.java`, `engine/excel/ExcelWorkbook.java`, `executor/src/test/java/dev/erst/gridgrind/executor/DefaultGridGrindRequestExecutorTest.java` | Core sheet lifecycle is fully surfaced. |
...
| Sheet protection including optional password hashes | Documented API support | `COMPLETE` | `contract/action/MutationAction.java`, `engine/excel/ExcelSheetStateController.java`, `engine/src/test/java/dev/erst/gridgrind/excel/ExcelSheetStateControllerTest.java`, `executor/src/test/java/dev/erst/gridgrind/executor/DefaultGridGrindRequestExecutorTest.java` | Lock flags and optional password-bearing sheet protection are both surfaced. |
...
| Sheet copy | Documented API support | `PARTIAL` | `contract/action/MutationAction.java`, `engine/excel/ExcelSheetCopyController.java`, `engine/src/test/java/dev/erst/gridgrind/excel/ExcelSheetCopyControllerTest.java`, `executor/src/parityTest/java/dev/erst/gridgrind/executor/parity/XlsxParityProbeRegistry.java` | GridGrind copies workbook-core structures that POI copies safely, including comments, tables, validations, conditional formatting, local formula-defined names, print layout, and protection metadata. Drawing-family content such as pictures and charts is not copied. |
...
| Capability | POI XSSF baseline | GridGrind status | GridGrind evidence | Notes |
|:-----------|:------------------|:-----------------|:-------------------|:------|
| Typed single-cell and range writes plus append row | Documented guide/API support | `COMPLETE` | `contract/dto/CellInput.java`, `contract/action/MutationAction.java`, `engine/excel/ExcelSheet.java`, `executor/src/test/java/dev/erst/gridgrind/executor/DefaultGridGrindRequestExecutorTest.java` | Shipped cell families are blank, text, rich text, number, boolean, formula, date, and date-time. |
...
| Cell snapshots, windows, schemas, and workbook or sheet summaries | Documented usermodel facts | `COMPLETE` | `contract/query/InspectionQuery.java`, `engine/excel/ExcelSheet.java`, `contract/dto/GridGrindResponse.java`, `executor/src/test/java/dev/erst/gridgrind/executor/DefaultGridGrindRequestExecutorTest.java` | This is the core factual read surface for exact workbook state. |
| Style patch and style snapshot reporting | Documented guide/API support | `COMPLETE` | `contract/dto/CellStyleInput.java`, `engine/excel/WorkbookStyleRegistry.java`, `engine/src/test/java/dev/erst/gridgrind/excel/ExcelCellStyleTest.java`, `executor/src/test/java/dev/erst/gridgrind/executor/DefaultGridGrindRequestExecutorTest.java` | Number formats, alignment, wrapping, borders, fills, font attributes, rotation, indentation, and protection flags ship. |
| Theme, indexed, tint, and gradient style semantics | POI low-level style support | `COMPLETE` | `contract/dto/ColorInput.java`, `contract/dto/CellGradientFillInput.java`, `engine/excel/WorkbookStyleRegistry.java`, `engine/src/test/java/dev/erst/gridgrind/excel/WorkbookStyleRegistryTest.java`, `executor/src/test/java/dev/erst/gridgrind/executor/DefaultGridGrindRequestExecutorTest.java` | GridGrind authors and reads structured color bases plus gradient fills without collapsing them to RGB-only approximations. |
| Scalar formula assignment plus `execution.calculation` policy | Documented formula and evaluation support | `COMPLETE` | `contract/dto/CellInput.java`, `contract/dto/CalculationPolicyInput.java`, `contract/dto/CalculationStrategyInput.java`, `engine/excel/ExcelWorkbook.java`, `executor/src/main/java/dev/erst/gridgrind/executor/CalculationPolicyExecutor.java`, `engine/src/test/java/dev/erst/gridgrind/excel/ExcelFormulaEnvironmentTest.java`, `executor/src/test/java/dev/erst/gridgrind/executor/FormulaEnvironmentRequestExecutorTest.java` | GridGrind keeps formula assignment as cell authoring and moves server-side calculation into `execution.calculation`. The shipped strategies are `EVALUATE_ALL`, `EVALUATE_TARGETS`, `CLEAR_CACHES_ONLY`, and `DO_NOT_CALCULATE`, with optional `markRecalculateOnOpen=true`. Request-authored array-formula braces such as `{=SUM(A1:A10*B1:B10)}` are rejected as `INVALID_FORMULA`. `LAMBDA` and `LET` are currently rejected as `INVALID_FORMULA` because Apache POI cannot parse them, and other newer constructs may fail the same way. Loaded formulas that POI parses but cannot evaluate surface as `UNSUPPORTED_FORMULA`. |
| Array-formula authoring and array-group metadata | XSSF array-formula APIs | `NOT_EXPOSED` | No protocol DTO or engine array-formula surface found; request-authored `{=...}` formulas fail validation in the public request path | GridGrind does not expose dedicated array-formula authoring, deletion, or array-group metadata. Existing array-formula cells may still appear only through generic scalar cell snapshots, without a dedicated public array-group contract. |
...
| Formula surface summaries | GridGrind-derived read over POI formula facts | `COMPLETE` | `contract/query/InspectionQuery.java`, `engine/excel/WorkbookAnalyzer.java`, `executor/src/test/java/dev/erst/gridgrind/executor/DefaultGridGrindRequestExecutorTest.java`, `examples/introspection-analysis-request.json` | `GET_FORMULA_SURFACE` returns `analysis.totalFormulaCellCount` plus per-sheet grouped formulas with `formulaCellCount`, `distinctFormulaCount`, and address lists, without mutating the workbook. |
...
| Capability | POI XSSF baseline | GridGrind status | GridGrind evidence | Notes |
|:-----------|:------------------|:-----------------|:-------------------|:------|
| Cell comments with author, visibility, rich-text runs, and anchors | Documented guide/API support | `COMPLETE` | `contract/dto/CommentInput.java`, `engine/excel/ExcelComment.java`, `engine/excel/ExcelCommentAnchor.java`, `engine/excel/ExcelCommentSnapshot.java`, `engine/excel/ExcelCommentAnchorSnapshot.java`, `engine/src/test/java/dev/erst/gridgrind/excel/ExcelCommentTest.java`, `engine/src/test/java/dev/erst/gridgrind/excel/ExcelRichTextSupportTest.java` | GridGrind authors plain text plus optional runs and optional anchor bounds, and factual reads surface those structures back. |
...
| Named-range authoring for explicit or formula-defined targets | Documented guide/API support | `COMPLETE` | `contract/dto/NamedRangeTarget.java`, `engine/excel/ExcelWorkbook.java`, `engine/src/test/java/dev/erst/gridgrind/excel/ExcelWorkbookTest.java`, `executor/src/test/java/dev/erst/gridgrind/executor/DefaultGridGrindRequestExecutorTest.java` | GridGrind writes workbook- and sheet-scoped names backed either by explicit ranges or by formulas. |
| Named-range factual reads | POI usermodel facts | `COMPLETE` | `contract/query/InspectionQuery.java`, `engine/excel/ExcelWorkbookIntrospector.java`, `executor/src/main/java/dev/erst/gridgrind/executor/WorkbookReadResultConverter.java`, `executor/src/test/java/dev/erst/gridgrind/executor/DefaultGridGrindRequestExecutorTest.java` | `GET_NAMED_RANGES` returns workbook- and sheet-scoped names backed by explicit ranges or formulas. |
| Named-range surface summaries | GridGrind-derived read over POI named-range facts | `COMPLETE` | `contract/query/InspectionQuery.java`, `engine/excel/WorkbookAnalyzer.java`, `executor/src/test/java/dev/erst/gridgrind/executor/DefaultGridGrindRequestExecutorTest.java`, `examples/introspection-analysis-request.json` | `GET_NAMED_RANGE_SURFACE` returns scope and backing counts plus per-name entries with `name`, `scope`, `refersToFormula`, and `kind`, without mutating the workbook. |
...
| Sheet schema inference | GridGrind-derived read over cell snapshots | `COMPLETE` | `contract/query/InspectionQuery.java`, `engine/excel/WorkbookAnalyzer.java`, `executor/src/test/java/dev/erst/gridgrind/executor/DefaultGridGrindRequestExecutorTest.java`, `examples/introspection-analysis-request.json` | `GET_SHEET_SCHEMA` infers header names and dominant types from a rectangular window. Formula cells contribute their evaluated result type, not `FORMULA`, to dominant-type reporting. |
