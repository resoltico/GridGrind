...
| Scalar formula assignment, workbook-wide evaluation, targeted evaluation, cache clearing, and recalc-on-open | Documented formula and evaluation support | `COMPLETE` | `protocol/dto/CellInput.java`, `protocol/dto/FormulaCellTargetInput.java`, `protocol/operation/WorkbookOperation.java`, `engine/excel/ExcelWorkbook.java`, `engine/src/test/java/dev/erst/gridgrind/excel/ExcelFormulaEnvironmentTest.java`, `protocol/src/test/java/dev/erst/gridgrind/protocol/exec/FormulaEnvironmentRequestExecutorTest.java` | GridGrind supports whole-workbook evaluation, explicit-cell evaluation, explicit persisted-cache clearing, and recalc-on-open for the scalar formula surface it models publicly. Request-authored array-formula braces such as `{=SUM(A1:A10*B1:B10)}` are rejected as `INVALID_FORMULA`. `LAMBDA` and `LET` are currently rejected as `INVALID_FORMULA` because Apache POI cannot parse them, and other newer constructs may fail the same way. Loaded formulas that POI parses but cannot evaluate surface as `UNSUPPORTED_FORMULA`. |
...
| Formula surface summaries | GridGrind-derived read over POI formula facts | `COMPLETE` | `protocol/read/WorkbookReadOperation.java`, `engine/excel/WorkbookAnalyzer.java`, `protocol/src/test/java/dev/erst/gridgrind/protocol/exec/DefaultGridGrindRequestExecutorTest.java`, `examples/introspection-analysis-request.json` | `GET_FORMULA_SURFACE` returns `analysis.totalFormulaCellCount` plus per-sheet grouped formulas with `formulaCellCount`, `distinctFormulaCount`, and address lists, without mutating the workbook. |
| Formula health analysis | GridGrind-derived analysis over POI formula facts | `COMPLETE` | `protocol/read/WorkbookReadOperation.java`, `engine/excel/WorkbookAnalyzer.java`, `engine/src/test/java/dev/erst/gridgrind/excel/WorkbookAnalyzerTest.java`, `examples/introspection-analysis-request.json` | `ANALYZE_FORMULA_HEALTH` returns `analysis.checkedFormulaCellCount`, a severity summary, and findings for volatile usage, formula-error results, and evaluation failures across the selected sheets. |

### Comments, Hyperlinks, Names, Tables, Filters, and Validation

| Capability | POI XSSF baseline | GridGrind status | GridGrind evidence | Notes |
|:-----------|:------------------|:-----------------|:-------------------|:------|
| Cell comments with author, visibility, rich-text runs, and anchors | Documented guide/API support | `COMPLETE` | `protocol/dto/CommentInput.java`, `engine/excel/ExcelComment.java`, `engine/excel/ExcelCommentAnchor.java`, `engine/excel/ExcelCommentSnapshot.java`, `engine/excel/ExcelCommentAnchorSnapshot.java`, `engine/src/test/java/dev/erst/gridgrind/excel/ExcelCommentTest.java`, `engine/src/test/java/dev/erst/gridgrind/excel/ExcelRichTextSupportTest.java` | GridGrind authors plain text plus optional runs and optional anchor bounds, and factual reads surface those structures back. |
...
| Hyperlink health analysis | GridGrind-derived analysis over POI hyperlink facts | `COMPLETE` | `protocol/read/WorkbookReadOperation.java`, `engine/excel/WorkbookAnalyzer.java`, `engine/src/test/java/dev/erst/gridgrind/excel/WorkbookAnalyzerTest.java`, `examples/file-hyperlink-health-request.json` | `ANALYZE_HYPERLINK_HEALTH` reports malformed, missing, unresolved, or broken targets. Relative `FILE` targets are resolved against the persisted workbook path when one exists. |
...
| Named-range factual reads | POI usermodel facts | `COMPLETE` | `protocol/read/WorkbookReadOperation.java`, `engine/excel/ExcelWorkbookIntrospector.java`, `protocol/exec/WorkbookReadResultConverter.java`, `protocol/src/test/java/dev/erst/gridgrind/protocol/exec/DefaultGridGrindRequestExecutorTest.java` | `GET_NAMED_RANGES` returns workbook- and sheet-scoped names backed by explicit ranges or formulas. |
| Named-range surface summaries | GridGrind-derived read over POI named-range facts | `COMPLETE` | `protocol/read/WorkbookReadOperation.java`, `engine/excel/WorkbookAnalyzer.java`, `protocol/src/test/java/dev/erst/gridgrind/protocol/exec/DefaultGridGrindRequestExecutorTest.java`, `examples/introspection-analysis-request.json` | `GET_NAMED_RANGE_SURFACE` returns scope and backing counts plus per-name entries with `name`, `scope`, `refersToFormula`, and `kind`, without mutating the workbook. |
| Named-range health analysis | GridGrind-derived analysis over POI named-range facts | `COMPLETE` | `protocol/read/WorkbookReadOperation.java`, `engine/excel/WorkbookAnalyzer.java`, `engine/src/test/java/dev/erst/gridgrind/excel/WorkbookAnalyzerTest.java`, `examples/introspection-analysis-request.json` | `ANALYZE_NAMED_RANGE_HEALTH` returns `analysis.checkedNamedRangeCount`, a severity summary, and findings for broken references, unresolved targets, and scope shadowing. |
...
| Sheet schema inference | GridGrind-derived read over cell snapshots | `COMPLETE` | `protocol/read/WorkbookReadOperation.java`, `engine/excel/WorkbookAnalyzer.java`, `protocol/src/test/java/dev/erst/gridgrind/protocol/exec/DefaultGridGrindRequestExecutorTest.java`, `examples/introspection-analysis-request.json` | `GET_SHEET_SCHEMA` infers header names and dominant types from a rectangular window. Formula cells contribute their evaluated result type, not `FORMULA`, to dominant-type reporting. |
| Aggregate workbook findings | GridGrind-derived aggregate over all shipped health families | `COMPLETE` | `protocol/read/WorkbookReadOperation.java`, `engine/excel/WorkbookAnalyzer.java`, `protocol/src/main/java/dev/erst/gridgrind/protocol/catalog/GridGrindProtocolCatalog.java`, `examples/workbook-health-request.json` | `ANALYZE_WORKBOOK_FINDINGS` runs formula, data-validation, conditional-formatting, autofilter, table, pivot-table, hyperlink, and named-range health across the workbook and returns one flat aggregated finding list. |

### Large-File Modes and Package Security

| Capability | POI XSSF baseline | GridGrind status | GridGrind evidence | Notes |
|:-----------|:------------------|:-----------------|:-------------------|:------|
| Streaming write with `SXSSF` | Documented by POI as a separate large-file mode | `COMPLETE` | `protocol/dto/ExecutionModeInput.java`, `protocol/exec/DefaultGridGrindRequestExecutor.java`, `engine/excel/ExcelStreamingWorkbookWriter.java`, `protocol/src/test/java/dev/erst/gridgrind/protocol/exec/ExecutionModeRequestExecutorTest.java`, `protocol/src/parityTest/java/dev/erst/gridgrind/protocol/parity/XlsxParityProbeRegistry.java`, `examples/large-file-modes-request.json` | GridGrind exposes `executionMode.writeMode=STREAMING_WRITE` for low-memory append-oriented authoring on `NEW` workbooks. The contract is intentionally narrow: only `ENSURE_SHEET`, `APPEND_ROW`, and `FORCE_FORMULA_RECALCULATION_ON_OPEN`. |
