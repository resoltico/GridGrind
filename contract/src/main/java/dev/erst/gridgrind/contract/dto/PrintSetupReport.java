...
public record PrintSetupReport(
  ...
  private static List<Integer> copyIndexes(List<Integer> indexes, String fieldName) {
    Objects.requireNonNull(indexes, fieldName + " must not be null");
    List<Integer> copy = new java.util.ArrayList<>(indexes.size());
    for (Integer index : indexes) {
      copy.add(Objects.requireNonNull(index, fieldName + " must not contain null indexes"));
    }
