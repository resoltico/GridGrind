#!/usr/bin/env bash
# Exercise the CLI discovery execution verifier against a fake jar launcher so long-running
...
printf '%s\n' "${output}" | grep -Fq 'Discovery execution examples 1/1: BUDGET printing request' || die \
    "discovery verifier no longer reports example request printing progress"
printf '%s\n' "${output}" | grep -Fq 'Discovery execution examples 1/1: BUDGET copying required assets' || die \
    "discovery verifier no longer reports example asset-copy progress"
...
printf '%s\n' "${output}" | grep -Fq 'Discovery execution tasks 1/1: DASHBOARD printing request' || die \
    "discovery verifier no longer reports task request printing progress"
printf '%s\n' "${output}" | grep -Fq 'Discovery execution tasks 1/1: DASHBOARD copying required assets' || die \
    "discovery verifier no longer reports task asset-copy progress"
